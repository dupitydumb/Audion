<script lang="ts">
  ...
  async function handlePlaylistContextMenu(playlist: Playlist, e: MouseEvent) {
      contextMenu.set({
          ...
          y: e.clientY,
          items: [
              { label: "Play", action: () => handlePlayPlaylist(playlist.id) },
              { label: "Add to Queue", action: () => handleAddToQueue(playlist.id) },
              { type: "separator" },
              {
                  label: "Rename",
...
<style>
  ...
  @media (max-width: 768px) {
      .playlist-view {
          padding-bottom: calc(var(--mobile-bottom-inset) + var(--spacing-md));
      }
