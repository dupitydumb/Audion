<script lang="ts">
  ...
  function measureQPOverflow(albumId: number) {
      ...
      requestAnimationFrame(() => {
          ...
          const nameOverflows = nameEl ? nameEl.scrollWidth > nameEl.clientWidth : false;
          const artistOverflows = artistEl ? artistEl.scrollWidth > artistEl.clientWidth : false;
          marqueeDurations = {
              ...marqueeDurations,
              [albumId]: {
                  name: nameEl && nameOverflows
                      ? `${Math.max(4, (nameEl.scrollWidth + MARQUEE_GAP) / 60).toFixed(1)}s`
                      : "0s",
                  artist: artistEl && artistOverflows
              ...
              {#each quickPlayAlbums as album}
                  ...
                  >
                      <div
                          ...
                          on:keydown|stopPropagation={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); playAlbum(album); } }}
                      >
                          ...
                          {#if album.artist}
                              <div class="qp-text-track" class:animate={active && overflows.artist}>
                                  <button
                                      ...
                                      class:qp-marquee={active && overflows.artist}
              ...
              {#each $recentlyPlayed.slice(0, 10) as track, i}
                  ...
                  >
                      <MediaCard
                          ...
                          secondaryAction={track.artist ? () => goToArtistDetail(track.artist!) : null}
